...
void GenIR::visit(StmtAST &ast) {
    switch (ast.sType) {
      ...
      requireLVal = true;
            ...
            auto left = recentVal;
            ast.exp->accept(*this);
            auto right = recentVal;
            if(left->type_ == FLOATPTR_T && right->type_ == INT32_T)
            {
                auto temp = builder->create_fptosi(right, FLOAT_T);
                builder->create_store(temp,left);
            }
            if(left->type_ == INT32PTR_T && right->type_ == FLOAT_T)
            {
                auto temp = builder->create_fptosi(right, INT32_T);
                builder->create_store(temp,left);
            }
            else{
