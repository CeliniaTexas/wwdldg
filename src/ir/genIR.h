...
//----------------------------------------------------
        std::vector<Type *> input_params;//声明
...
//----------------------------------------------------
        std::vector<Type *> output_params;//声明
        std::vector<Type *>().swap(output_params);
        output_params.push_back(TyInt32);
        auto output_type = new FunctionType(TyVoid, output_params);//创建指针
        auto put_int = new Function(output_type, "putint", module.get());//创建指针
